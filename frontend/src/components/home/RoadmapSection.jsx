...
const STEPS = [
  {
    ...
    description: "Understand the true potential of your idea through structured analysis:",
    bullets: ["Patentability & IP scope", "Technology Readiness (TRL)", "Market potential", "Commercialization pathways"],
    outcome: "Clear direction. Reduced risk. Defined next step.",
    cta: { label: "Start with Assessment", to: "/reports" },
    icon: FlaskConical,
    color: "#3b82f6",
    gradient: "from-blue-500 to-blue-700",
    light: "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800/40",
    dot: "bg-blue-600",
  ...
  },
  {
    ...
    bullets: ["POC, Prototype, MVP", "IoT, AI, Medical, Software solutions", "Scalable and test-ready builds"],
    outcome: "A functional product — not just an idea.",
    cta: { label: "Build Your Prototype", to: "/prototype" },
    icon: Wrench,
    color: "#14b8a6",
    gradient: "from-teal-500 to-teal-700",
    light: "bg-teal-50 dark:bg-teal-900/20 border-teal-200 dark:border-teal-800/40",
    dot: "bg-teal-600",
  ...
  },
  {
    ...
    bullets: ["Minimum POC / MVP required", "Real validation & use-case clarity", "Founder commitment check"],
    outcome: "Entry into a high-quality ecosystem.",
    cta: { label: "Check Eligibility", to: "/technologies" },
    icon: ShieldCheck,
    color: "#10b981",
    gradient: "from-emerald-500 to-emerald-700",
    light: "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/40",
    dot: "bg-emerald-600",
  ...
  },
  {
    ...
    bullets: ["Financial capacity validation", "Investment experience check", "Sector and strategy alignment"],
    outcome: "Access to credible, active investors.",
    cta: { label: "Join as Investor", to: "/investors" },
    icon: BadgeDollarSign,
    color: "#f59e0b",
    gradient: "from-amber-500 to-amber-600",
    light: "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/40",
    dot: "bg-amber-500",
  ...
  },
  {
    ...
    bullets: ["Stage compatibility", "Sector alignment", "Ticket size", "Risk and strategic fit"],
    outcome: "High-relevance investor–startup connections.",
    cta: { label: "Get Matched", to: "/investors" },
    icon: Zap,
    color: "#8b5cf6",
    gradient: "from-violet-500 to-violet-700",
    light: "bg-violet-50 dark:bg-violet-900/20 border-violet-200 dark:border-violet-800/40",
    dot: "bg-violet-600",
  ...
  },
  {
    ...
    bullets: ["Direct investor interaction", "Funding discussions", "Growth and commercialization"],
    outcome: "Capital + scale + market entry.",
    cta: null,
    icon: TrendingUp,
    color: "#f43f5e",
    gradient: "from-rose-500 to-rose-600",
    light: "bg-rose-50 dark:bg-rose-900/20 border-rose-200 dark:border-rose-800/40",
    dot: "bg-rose-500",
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 pt-1">
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${step.light} ${step.text}`}>
          {step.tag}
        </span>
        <div
      ...
      {/* Outcome */}
      <div
        className={`rounded-lg px-3 py-2 mb-4 border ${step.light}`}
      >
        <p className="text-xs leading-relaxed">
          <span className={`font-semibold ${step.text}`}>Outcome: </span>
          <span className="text-slate-600 dark:text-slate-300">{step.outcome}</span>
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:block relative">
          ...
          <div className="absolute left-1/2 -translate-x-px top-0 bottom-0 w-px">
            <div className="w-full h-full bg-gradient-to-b from-blue-500 via-violet-500 to-rose-500 opacity-30" />
          </div>
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <div key={i} className="relative grid grid-cols-2 gap-0 items-center">
                  {/* Left side */}
                  <div className={`${isRight ? "pr-16 flex justify-end" : "pr-16"}`}>
                    {!isRight && <RoadmapCard step={step} index={i} isRight={false} />}
                  </div>
                  ...
                  >
                    <div
                      className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl border-2 border-slate-800"
                      style={{ background: `linear-gradient(135deg, ${step.color}dd, ${step.color})` }}
                    >
                      <Icon className="h-6 w-6 text-white" />
                    </div>
                    <span className="mt-1.5 text-xs font-black text-slate-500 tracking-widest">
                      {step.number}
                    </span>
                  </motion.div>

                  {/* Right side */}
                  <div className={`${!isRight ? "pl-16 flex justify-start" : "pl-16"}`}>
                    {isRight && <RoadmapCard step={step} index={i} isRight={true} />}
                  </div>
                </div>
              );
            })}
        ...
        <div className="lg:hidden relative pl-12">
          {/* Left vertical line */}
          <div className="absolute left-4 top-0 bottom-0 w-px bg-gradient-to-b from-blue-500 via-violet-500 to-rose-500 opacity-40" />
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                ...
                >
                  {/* Node on the rail */}
                  <div
                    className="absolute -left-12 top-4 w-9 h-9 rounded-xl flex items-center justify-center shadow-lg border-2 border-slate-800"
                    style={{ background: `linear-gradient(135deg, ${step.color}cc, ${step.color})` }}
                  >
                    <Icon className="h-4 w-4 text-white" />
                  </div>
                  ...
                  <div className="bg-slate-900 rounded-2xl border border-slate-700 p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${step.light} ${step.text}`}>
                    ...
                    <ul className="space-y-1 mb-3">
                      {step.bullets.map((b, j) => (
                        <li key={j} className="flex items-start gap-2 text-sm text-slate-400">
                          <span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: step.color }} />
                          {b}
                        </li>
                      ))}
                    </ul>
        ...
        >
          {STRIP.map((item, i) => (
            <div key={i} className="flex items-center">
              ...
              {i < STRIP.length - 1 && (
                <div className="w-10 sm:w-16 mx-1 mb-5 h-px bg-gradient-to-r from-slate-600 to-slate-700" />
              )}
            </div>
          ))}
        </motion.div>
